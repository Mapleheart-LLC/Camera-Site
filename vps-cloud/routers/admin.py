...
def _post_answer_bluesky(question_id: str, answer_text: str) -> bool:
    ...
    try:
        ...
        logger.info("Answer posted to Bluesky for question %s", question_id)
        return True
    except Exception as exc:
...
class _CreateUserPayload(BaseModel):
    ...
    role: str = Field("user", pattern=r"^(user|paid_user|handler|admin)$")


class _UpdateUserPayload(BaseModel):
    password: Optional[str] = Field(None, min_length=8, max_length=72)
    access_level: Optional[int] = Field(None, ge=0, le=3)
    role: Optional[str] = Field(None, pattern=r"^(user|paid_user|handler|admin)$")
